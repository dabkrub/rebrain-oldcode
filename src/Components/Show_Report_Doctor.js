...
function Show_Report_Doctor() {
  ...
  const onFetchFirstCase_Report = () => {
    ...
    fectData.orderByKey().on("child_added", (snapshort) => {
      const Check = firebase
        ...
        .child(snapshort.key);
      Check.on("value", (snap) => {
        if (snap.val().Doctor_ID == Doctor_ID) {
    ...
    console.log("DataFirstList", DataList_FirstCase);
  };
  ...
  function onFetchSelect(e) {
    ...
    for (let i in DataList_FirstCase) {
      if (DataList_FirstCase[i].CaseName == e) {
    ...
    for (let i in DataList_Month) {
      if (DataList_Month[i].CaseName == e) {
...
export default Show_Report_Doctor;
